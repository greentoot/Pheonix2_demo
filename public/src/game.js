(async function(){
  ...
  const enemyImg = await loadImage('./src/assets/enemies/sparrows/sparrows_unprotected.png'); // sprite ennemi
  const statsparrows = await loadJSON('./src/assets/enemies/sparrows/data.json');  //statsvaisseaux enenemie
  let healthennemy = statsparrows.textContent = statsparrows['Health'] || 20;
  let Firespeed = statsparrows.textContent = statsparrows['Rate of Fire'] || 1.5;
  // Game state
  let running = true;
  let score = 0;
  let lives = 1;
  ...
  function spawnWave(){
    const pattern = Math.random() < 0.5 ? 'line' : 'vee';
    ...
    for(let i=0;i<count;i++){
      ...
      enemies.push({ x, y, w: 48, h: 48, hp: 2, vx: (Math.random()-.5)*30, vy: 60 + Math.random()*30, fire: Math.random()*1.5 + .5 });
    }
  }

  function shoot(){
    bullets.push({ x: player.x, y: player.y-24, vy: -player.projectileSpeed });
    bullets.push({ x: player.x+10, y: player.y-24, vy: -player.projectileSpeed*0.98 });
    bullets.push({ x: player.x-10, y: player.y-24, vy: -player.projectileSpeed*0.98 });
  }
  ...
  function update(dt){
    // clamp position pour ne pas sortir de l'écran
  ...
  function draw(){
    ...
    for(const b of bullets){
      glowCircle(b.x, b.y, 3);
    }
    ctx.restore();

    // 🔹 Dessin des ennemis avec sprite
    for(const e of enemies){
      ctx.save();
  ...
  function glowCircle(x,y,r){
    ...
    g.addColorStop(0.5,'rgba(120,180,255,.6)');
